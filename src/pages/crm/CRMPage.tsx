...
const AddReferralModal: React.FC<{ open: boolean; onClose: () => void; onSaved: () => void; editDoc?: any }> = ({ open, onClose, onSaved, editDoc }) => {
  ...
  const save = async () => {
    if (!form.doctor_name || !form.phone) { toast({ title: "Name and phone required", variant: "destructive" }); return; }
    const payload = { ...form, hospital_id: HOSPITAL_ID };
    const { error } = editDoc
