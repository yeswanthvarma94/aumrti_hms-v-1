...
const OPDPage: React.FC = () => {
  ...
  const fetchTokens = useCallback(async () => {
    ...
    setUserId(user.id);
    const { data: userData, error: userErr } = await supabase.from("users").select("hospital_id")const { data: userData, error: userErr } = await supabase.from("users").select("hospital_id").eq("auth_user_id", user.id).single();
