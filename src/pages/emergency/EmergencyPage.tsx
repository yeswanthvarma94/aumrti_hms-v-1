...
const EmergencyPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setUserId(user.id);
    const { data: ud, error: udErr } = await supabase.from("users").select("hospital_id")const { data: ud, error: udErr } = await supabase.from("users").select("hospital_id").eq("auth_user_id", user.id).single();
