    ...
    switch (activeTab) {
      ...
      case "reports": return (
        <div className="flex-1 flex items-center justify-center text-muted-foreground text-sm">
          Reports module coming in the next phase.
        </div>
      );
    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 56px)" }}>
      ...
      <div className="h-[52px] flex-shrink-0 bg-card border-b border-border flex items-center justify-between px-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className={cn("text-xs px-2.5 py-1 rounded-full font-medium", kpis.lowStock > 0 ? "bg-destructive/10 text-destructive" : "bg-success/10 text-success")}>
            🔴 {kpis.lowStock} Low Stock
          </span>
          <span className={cn("text-xs px-2.5 py-1 rounded-full font-medium", kpis.expiring > 0 ? "bg-accent/10 text-accent-foreground" : "bg-success/10 text-success")}>
